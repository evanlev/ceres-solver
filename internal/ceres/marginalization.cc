...
static ProblemImpl* BuildProblemWithMarginalizedBlocksAndMarkovBlanket(
    ...
    const bool use_tangent_space_formulation,
  ...
  for (auto it = parameter_blocks_to_marginalize.begin();
    ...
    for (size_t j = 0; j < residual_blocks.size(); ++j) {
      ...
      for (size_t k = 0; k < parameter_blocks.size(); ++k) {
        ...
        const bool add_parameterization = use_tangent_space_formulation ||
          static_cast<bool>(
...
static bool ComputeMarginalFactorData(
  ...
  local_problem.reset(BuildProblemWithMarginalizedBlocksAndMarkovBlanket(
      external_problem, options.use_tangent_space_formulation,
  ...
  *blanket_parameter_blocks =
  ...
  const int size_blanket =
      options.use_tangent_space_formulation
          ? SumLocalSize(*blanket_parameter_blocks)
          : SumGlobalSize(*blanket_parameter_blocks);
  ...
  *jacobian = sqrt_factor.transpose();
  if (options.use_tangent_space_formulation) {
    *b = *jacobian * ldlt.solve(gradient_marginal_factor);
  } else {
    *b = *jacobian * (ldlt.solve(gradient_marginal_factor) -
                                    state_vector.tail(size_blanket));
  }
  return true;
}
...
static bool ComputeWithGlobalSpaceFormulation(
    const MarginalizationOptions& options,
    const set<double*>& parameter_blocks_to_marginalize,
    const map<double*, const double*>* parameter_block_linearization_states,
    Problem* problem, vector<double*>* blanket_parameter_blocks_state,
  ...
  vector<int> blanket_parameter_block_sizes(blanket_parameter_blocks.size());
  std::transform(blanket_parameter_blocks.begin(),
                 blanket_parameter_blocks.end(),
                 blanket_parameter_block_sizes.begin(),
                 [](ParameterBlock* pb) { return pb->Size(); });

  blanket_parameter_blocks_state->resize(blanket_parameter_blocks.size());
  std::transform(blanket_parameter_blocks.begin(),
                 blanket_parameter_blocks.end(),
                 blanket_parameter_blocks_state->begin(),
                 [](ParameterBlock* pb) { return pb->mutable_user_state(); });

  *marginalization_prior_cost_function =
      new LinearCostFunction(marginal_factor_jacobian, marginal_factor_b,
                             blanket_parameter_block_sizes);
...
static bool MarginalizeOutVariables(
  ...
  auto compute_fptr = options.use_tangent_space_formulation
                          ? ComputeWithTangentSpaceFormulation
                          : ComputeWithGlobalSpaceFormulation;

  if (!compute_fptr(options,
                    parameter_blocks_to_marginalize,
